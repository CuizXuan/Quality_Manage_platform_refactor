...
def list_folders(tenant_id: int = Depends(get_tenant_id), db: Session = Depends(get_db)):
...
def create_folder(data: FolderCreate, tenant_id: int = Depends(get_tenant_id), db: Session = Depends(get_db)):
    folder = CaseFolder(**data.model_dump(), tenant_id=tenant_id)
    db.add(folder)
    db.commit()
    db.refresh(folder)
    return folder
...
def update_folder(folder_id: int, data: FolderUpdate, tenant_id: int = Depends(get_tenant_id), db: Session = Depends(get_db)):
...
def delete_folder(folder_id: int, tenant_id: int = Depends(get_tenant_id), db: Session = Depends(get_db)):
    folder = db.query(CaseFolder).filter(
        CaseFolder.id == folder_id,
        CaseFolder.tenant_id == tenant_id
    ).first()
